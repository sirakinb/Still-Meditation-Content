#!/bin/bash
# Regenerate Day 60 slide 4 — typo fix "quiter" -> "quieter"
...
# Slide 4 — Heart rate (typo fix). Render text exactly. Shorter body lines.
...
Render this text EXACTLY as written. Spell every word correctly.
Large bold left-aligned ALL-CAPS section header at the top reads exactly: HEART RATE
LEAVE A CLEAR GAP between header and body.
Below in medium bold text, three lines exactly:
Resting rate dropped.
Body runs quieter.
Calmer all day.
Triple-check the word QUIETER is spelled Q-U-I-E-T-E-R (not 'quiter')."

echo ""
echo "=== Day 60 fix complete ==="
