#!/bin/bash
# Post Day 21 to PostBridge: TikTok as native draft, Instagram as straight post
set -e
...
TIKTOK_ACCOUNT=57672
INSTAGRAM_ACCOUNT=57667

TIKTOK_CAPTION=$(python3 -c "
import re
...
echo "=== TikTok caption (first 100 chars): ${TIKTOK_CAPTION:0:100}..."
...
echo ""
echo "=== Uploading slides 1-6 (shared) ==="
MEDIA_IDS=()
...
echo ""
echo "=== Uploading slide 7 (TikTok) ==="
MID_TT7=$(upload_media "$SLIDES_DIR/slide7.png" "day${DAY}_slide7_tiktok.png")
echo "slide7 TikTok: $MID_TT7"
...
TT_MEDIA=$(printf '"%s",' "${MEDIA_IDS[@]}")
TT_MEDIA="[${TT_MEDIA}\"${MID_TT7}\"]"

IG_MEDIA=$(printf '"%s",' "${MEDIA_IDS[@]}")
IG_MEDIA="[${IG_MEDIA}\"${MID_IG7}\"]"

echo ""
echo "=== Creating TikTok draft post ==="
TT_PAYLOAD=$(jq -n \
  --arg caption "$TIKTOK_CAPTION" \
  --argjson media "$TT_MEDIA" \
  --argjson acct "$TIKTOK_ACCOUNT" \
  '{
    caption: $caption,
    media: $media,
...
TT_RESP=$(curl -s -X POST "$PB_URL/v1/posts" \
  -H "Authorization: Bearer $PB_KEY" \
  -H "Content-Type: application/json" \
  -d "$TT_PAYLOAD")
echo "TikTok response:"
echo "$TT_RESP" | jq '.'
...
echo ""
echo "=== Day ${DAY} PostBridge complete ==="
