#!/bin/bash
# Regen Day 76 slide 4 only — fix duplicated "ASK" in headline
...
# Slide 4 — STEP 2: ASK (fixed — headline must appear EXACTLY ONCE as two words only)
generate_slide 4 "Create a bold TikTok social media card illustration. ${STYLE}
Layout: Text occupies the LEFT 60% of the card. The mascot is on the FAR RIGHT, fully inside the right 40%, with NOTHING in the left text zone.
The scene: The mascot sits cross-legged with one hand gently pressed to his temple, eyes lightly closed, a look of quiet inward turning on his face.
CRITICAL TEXT RULE: The headline must appear EXACTLY as shown here — no repetition, no duplication of any word:
Large bold left-aligned headline text at the very top reads EXACTLY these two lines only — spelled correctly, printed once, no extra words:
STEP 2: ASK
LEAVE A CLEAR GAP between headline and body text.
Below the gap in medium bold text, print EXACTLY these lines spelled correctly:
Silently ask:
